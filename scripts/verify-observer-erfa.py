    ...
    with SPK.open(path) as kernel:
        ...
        for name in ("moon-singapore", "venus-greenwich", "sun-singapore"):
            ...
            for line in text.split("$$SOE")[1].split("$$EOE")[0].strip().splitlines():
                ...
                utc = erfa.dtf2d("UTC", instant.year, instant.month, instant.day, instant.hour, instant.minute, instant.second)
                ...
                tau = 0.
                for _ in range(16):
                    tp, _ = state(target, jd-tau/86400)
                    p = tp-a["eb"]
