...
import { columns, endpoint, parseCount, parseSources, queries, sha256, spatialChunks } from './lib/gaia-dr3.mjs'

export async function retrieveGaiaCone(settings, { fetcher = fetch, signal } = {}) {
  const query = queries(settings)
  ...
  const rows = parseSources(rowSource.bytes, settings, count); signal?.throwIfAborted()
  return { countSource, rowSource, rows, chunks: spatialChunks(rows) }
}
async function main() {
  const args = process.argv.slice(2), known = ['--ra', '--dec', '--radius', '--max-mag', '--max-rows', '--output'], values = {}
  if (args.length !== known.length*2) throw new Error('Required: --ra --dec --radius --max-mag --max-rows --output (new directory)')
  ...
  try {
    const result = await retrieveGaiaCone(settings, { signal: controller.signal }), directory = values['--output']
    ...
    const implementation = { node: process.version, generatorSha256: sha256(await readFile(new URL(import.meta.url))), parserSha256: sha256(await readFile(new URL('./lib/gaia-dr3.mjs', import.meta.url))), columnsSha256: sha256(await readFile(new URL('../src/data/gaiaColumns.json', import.meta.url))) }
    const manifest = { schemaVersion: 1, catalog: 'Gaia DR3', table: 'gaiadr3.gaia_source', frame: 'ICRS', referenceEpochJulianYear: 2016, referenceEpochTimeScale: 'TCB', implementation, settings, columns, rows: result.rows.length, queryCountMatched: true, catalogCompletenessCertified: false,
      units: { ra: 'degree', dec: 'degree', ra_error: 'mas; alpha*cos(delta)', dec_error: 'mas', parallax: 'mas', pmra: 'mas/Julian year; mu_alpha*cos(delta)', pmdec: 'mas/Julian year', radial_velocity: 'km/s' },
