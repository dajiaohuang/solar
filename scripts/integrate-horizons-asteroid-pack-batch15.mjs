...
// model is introduced. Hektor and Agamemnon are primary heliocentric states;
// this pack does not claim companion/system-center coverage.
import { createHash } from 'node:crypto'
import { mkdir, readFile, writeFile } from 'node:fs/promises'
import { join } from 'node:path'
import { cropSpk } from './crop-spk.mjs'

const cache = process.env.SOLAR_HORIZONS_ASTEROID_BATCH15_CACHE ?? '.cache/horizons-asteroids-batch15-20260909'
const output = 'public/data/ephemerides'
const batch = 'horizons-asteroids-batch15-20260909'
const coreId = 'de440s-2000-01-01-2051-01-01'
const targets = [
  ['eunomia', 'Eunomia', 15, 20000015],
  ['thisbe', 'Thisbe', 88, 20000088],
  ['hektor', 'Hektor', 624, 20000624],
  ['agamemnon', 'Agamemnon', 911, 20000911],
]
  ...
  for (const [profile, [from, to]] of Object.entries(profileWindows)) {
    ...
    entries[profile] = {
      ...
      selectionEvidence: {
        ...
        stateBoundary: (name === 'Hektor' || name === 'Agamemnon')
          ? 'Primary heliocentric state only; companion/system-center coverage is not included.'
          : undefined,
      },
    }
