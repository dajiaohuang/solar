// Integrate the next deliberately bounded, locally frozen NASA/JPL Horizons
// SPK pack. The cache is the release input: every source response carries
// binary and response hashes before publication.
import { createHash } from 'node:crypto'
import { mkdir, readFile, writeFile } from 'node:fs/promises'
import { join } from 'node:path'
import { cropSpk } from './crop-spk.mjs'

const cache = process.env.SOLAR_HORIZONS_ASTEROID_NEXT_CACHE ?? '.cache/horizons-asteroids-next-20260909'
const output = 'public/data/ephemerides'
const batch = 'horizons-asteroids-next-20260909'
