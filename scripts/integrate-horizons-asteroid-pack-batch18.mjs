...
// Original type-21 records are retained; no refit, extrapolation or N-body
// model is introduced. These direct asteroid targets are primary heliocentric
// states only; this pack does not claim companion/system-center coverage.
import { createHash } from 'node:crypto'
import { mkdir, readFile, writeFile } from 'node:fs/promises'
import { join } from 'node:path'
import { cropSpk } from './crop-spk.mjs'

const cache = process.env.SOLAR_HORIZONS_ASTEROID_BATCH18_CACHE ?? '.cache/horizons-asteroids-batch18-20260910'
const output = 'public/data/ephemerides'
const batch = 'horizons-asteroids-batch18-20260910'
const coreId = 'de440s-2000-01-01-2051-01-01'
const targets = [
  ['pallas', 'Pallas', 2, 20000002, 2000002],
  ['vesta', 'Vesta', 4, 20000004, 2000004],
  ['hygiea', 'Hygiea', 10, 20000010, 2000010],
  ['psyche', 'Psyche', 16, 20000016, 2000016],
]
...
for (const [slug, name, designation, target, legacyTarget] of targets) {
  ...
  for (const [profile, [from, to]] of Object.entries(profileWindows)) {
    ...
    const existing = manifest.files.filter(file => file.targets?.includes(target) || file.targets?.includes(legacyTarget))
    ...
    entries[profile] = {
      ...
      selectionEvidence: {
        ...
        stateBoundary: 'Primary heliocentric state only; companion/system-center coverage is not included.',
      },
    }
    for (const oldFile of existing) manifest.files.splice(manifest.files.indexOf(oldFile), 1)
