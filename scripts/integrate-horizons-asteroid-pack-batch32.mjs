// Integrate locally frozen NASA/JPL Horizons type-21 SPKs for named Centaurs.
import { createHash } from 'node:crypto'
import { mkdir, readFile, writeFile } from 'node:fs/promises'
import { join } from 'node:path'
import { cropSpk } from './crop-spk.mjs'

const cache = process.env.SOLAR_HORIZONS_ASTEROID_BATCH32_CACHE ?? '.cache/horizons-centaur-batch32-20260914'
const output = 'public/data/ephemerides'
const batch = 'horizons-centaur-batch32-20260914'
const coreId = 'de440s-2000-01-01-2051-01-01'
const targets = [
  ['petraios', '696315 Petraios', '696315', 20696315],
  ['xewioso', '78799 Xewioso', '78799', 20078799],
  ['goibniu', '90568 Goibniu', '90568', 20090568],
]
...
const profileWindows = { pages: ['2026-01-01', '2027-01-01'], full: ['2020-01-01', '2031-01-01'] }
...
for (const [slug, name, designation, target] of targets) {
  ...
  if (source.source?.startsWith('https://') !== true || source.target !== target
    || source.designation !== designation || source.from !== '2020-01-01' || source.to !== '2031-01-01'
  ...
  for (const [profile, [from, to]] of Object.entries(profileWindows)) {
    ...
    entries[profile] = {
      ...
      selectionEvidence: {
        sourceSelection: `${name} frozen from one NASA/JPL Horizons SPK response; original type-21 records retained, no refitting.`,
