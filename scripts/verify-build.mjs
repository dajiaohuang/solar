...
if (spkDelivery?.ephemerisBodyCount !== 52 || spkDelivery?.planetarySatelliteBodyCount !== 31 || spkDelivery?.smallBodyBodyCount !== 21 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== '8e29cfb40a957dcaee094d09ed494900675caf3feecb15937c6ef3cb6694ca14' || spkDelivery?.pagesManifest?.bytes !== 271043584 || spkDelivery?.pagesManifest?.fileCount !== 516) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== '9058cd96aa87a2e5c24bd071ae72e6edf8637953167301f27a9254ec70b56258' || spkDelivery?.fullManifest?.bytes !== 1149197312 || spkDelivery?.fullManifest?.fileCount !== 516) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
