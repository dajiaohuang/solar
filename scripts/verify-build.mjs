...
if (spkDelivery?.ephemerisBodyCount !== 117 || spkDelivery?.planetarySatelliteBodyCount !== 31 || spkDelivery?.smallBodyBodyCount !== 86 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== 'b2dd6520a54b48a838a6044cc3a2a155ded54a25a88087443ca5b3bb44930ef4' || spkDelivery?.pagesManifest?.bytes !== 272689152 || spkDelivery?.pagesManifest?.fileCount !== 590) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== 'b91c2a4cc84e35de18a12952c1b17884c18d2f9bc82b5be9f934e1bc282a219d' || spkDelivery?.fullManifest?.bytes !== 1164678144 || spkDelivery?.fullManifest?.fileCount !== 590) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
