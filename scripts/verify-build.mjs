...
if (spkDelivery?.ephemerisBodyCount !== 314 || spkDelivery?.planetarySatelliteBodyCount !== 152 || spkDelivery?.smallBodyBodyCount !== 162 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== 'b96ab54de5210d9e4b0ec262133a07f3ded3bb264614f130f359ab366a5efcbe' || spkDelivery?.pagesManifest?.bytes !== 274358272 || spkDelivery?.pagesManifest?.fileCount !== 668) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== 'f4da36c79981c0e8a28b003a111afc1bfa74017a8d359d69cdc1d6c924ac95cd' || spkDelivery?.fullManifest?.bytes !== 1181386752 || spkDelivery?.fullManifest?.fileCount !== 668) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
