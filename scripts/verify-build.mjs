...
if (spkDelivery?.ephemerisBodyCount !== 268 || spkDelivery?.planetarySatelliteBodyCount !== 152 || spkDelivery?.smallBodyBodyCount !== 116 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== '6a71fcd4b0455e7db41979b1300f3e5436477364fafe056d2abf89ac34d0accf' || spkDelivery?.pagesManifest?.bytes !== 273356800 || spkDelivery?.pagesManifest?.fileCount !== 622) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== '6c0869250e035add7cdb415320831efece813e00b594bbefc0bdd16051a373a6' || spkDelivery?.fullManifest?.bytes !== 1171943424 || spkDelivery?.fullManifest?.fileCount !== 622) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
