...
if (spkDelivery?.ephemerisBodyCount !== 69 || spkDelivery?.planetarySatelliteBodyCount !== 31 || spkDelivery?.smallBodyBodyCount !== 38 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== '08dc71abcc3aff371adc4103c41f137fd07569c4ffb6972590ba0e7a07f2325f' || spkDelivery?.pagesManifest?.bytes !== 271421440 || spkDelivery?.pagesManifest?.fileCount !== 533) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== 'e4df952dd4f5ed07fcb0666158dfcfe12c000084b5e6e97b8ecc9cf2d635c031' || spkDelivery?.fullManifest?.bytes !== 1152739328 || spkDelivery?.fullManifest?.fileCount !== 533) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
...
if (JSON.stringify(spkDelivery?.frozenHorizonsAsteroids) !== JSON.stringify(['243', '433', '951', '25143', '99942', '162173', '3200', '3122', '65803', '4179', '1036', '1580', '2867', '52768', '29075', '231937', '6', '9', '14', '18', '19', '90', '216'])) throw new Error('Scientific validation report is missing the frozen Horizons asteroid batch')
