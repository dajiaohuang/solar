...
if (spkDelivery?.ephemerisBodyCount !== 112 || spkDelivery?.planetarySatelliteBodyCount !== 31 || spkDelivery?.smallBodyBodyCount !== 81 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== '51c6095c29938944b4982d7014f46ec3b08eecd7e3395a1591526cb0b1586553' || spkDelivery?.pagesManifest?.bytes !== 272577536 || spkDelivery?.pagesManifest?.fileCount !== 585) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== '4c7c383fe016a1a314771a0289742ff0f236d12fcfc6a6cb769fd3f44acf2d8e' || spkDelivery?.fullManifest?.bytes !== 1163642880 || spkDelivery?.fullManifest?.fileCount !== 585) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
