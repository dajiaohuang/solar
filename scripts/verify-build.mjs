...
if (spkDelivery?.ephemerisBodyCount !== 258 || spkDelivery?.planetarySatelliteBodyCount !== 152 || spkDelivery?.smallBodyBodyCount !== 106 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== '69554f58f7fea3eefdf1ff450dc1cbbf1331936225b74d73d1d0d99a3ab3bb62' || spkDelivery?.pagesManifest?.bytes !== 273137664 || spkDelivery?.pagesManifest?.fileCount !== 612) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== '94ca52e32a52145c61b91d56a78d2cfbc3f123490f5c33b49c25f067209e8f27' || spkDelivery?.fullManifest?.bytes !== 1169903616 || spkDelivery?.fullManifest?.fileCount !== 612) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
