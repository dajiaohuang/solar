...
if (spkDelivery?.ephemerisBodyCount !== 276 || spkDelivery?.planetarySatelliteBodyCount !== 152 || spkDelivery?.smallBodyBodyCount !== 124 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== '7cb8d902d32ddfee046e4afc5b8fcf2de75d49708f5dded14e9d2f7903dcf179' || spkDelivery?.pagesManifest?.bytes !== 273528832 || spkDelivery?.pagesManifest?.fileCount !== 630) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== '67159d8d19f002a62b47f48fd251bb463fc7e6f7e216dd2e4db21a3b86ee108b' || spkDelivery?.fullManifest?.bytes !== 1173569536 || spkDelivery?.fullManifest?.fileCount !== 630) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
