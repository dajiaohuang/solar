...
if (spkDelivery?.ephemerisBodyCount !== 293 || spkDelivery?.planetarySatelliteBodyCount !== 152 || spkDelivery?.smallBodyBodyCount !== 141 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== 'ecfd7964aee2a526f80bb0791d75edcefb35049511bdab233a37a5e574183791' || spkDelivery?.pagesManifest?.bytes !== 273900544 || spkDelivery?.pagesManifest?.fileCount !== 647) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== 'd6a6b125d61ef4a86fe6cce15e55227b1f55fa0aeb8e502e3abc7da464917eac' || spkDelivery?.fullManifest?.bytes !== 1177074688 || spkDelivery?.fullManifest?.fileCount !== 647) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
