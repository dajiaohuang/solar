...
if (spkDelivery?.ephemerisBodyCount !== 306 || spkDelivery?.planetarySatelliteBodyCount !== 152 || spkDelivery?.smallBodyBodyCount !== 154 || spkDelivery?.satelliteIdentityCount !== 472) throw new Error('Scientific validation report is missing the expanded SPK coverage counts')
...
if (spkDelivery?.pagesManifest?.id !== 'jpl-satellite-expansion-20260904-pages' || spkDelivery?.pagesManifest?.sha256 !== '48b3e0e270119ec9d957f1e0c210338afef5889284a2c32328cd790b125dff9b' || spkDelivery?.pagesManifest?.bytes !== 274185216 || spkDelivery?.pagesManifest?.fileCount !== 660) throw new Error('Scientific validation report is missing the pinned Pages SPK manifest')
if (spkDelivery?.fullManifest?.id !== 'jpl-satellite-expansion-20260904-full' || spkDelivery?.fullManifest?.sha256 !== 'cd4b194d15d7ff3a146a5c674934c28cb3d4b9956795c704d396deaf30f7e1f5' || spkDelivery?.fullManifest?.bytes !== 1179769856 || spkDelivery?.fullManifest?.fileCount !== 660) throw new Error('Scientific validation report is missing the pinned full SPK manifest')
