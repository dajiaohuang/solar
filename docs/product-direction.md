...
3. Build independent Android and iOS clients against that same contract. Each must demonstrate the Observation Deck, preset selection, tutorial, object evidence and cache/error handling with platform-appropriate interaction / Android 与 iOS 对接同一协议，各自交付综合看板、预设、教程、天体证据及缓存/错误处理，采用符合平台习惯的交互。
4. Publish and verify the same-Web-frontend Pages profile separately from full Web and native builds; audit enabled entries, visible full-only entries, accessible explanations, blocked actions/downloads and direct-link handling / 将共用 Web 前端的 Pages 配置与完整 Web、原生构建分别发布验证，审计可用入口、可见完整版入口、无障碍说明、受限操作/下载拦截及直接链接处理。
