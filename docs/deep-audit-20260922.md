...
- Web CI: lint, 653 passing unit tests with 9 conditional skips, 161/161 scientific tests, TypeScript and production artifact validation passed. The exact-head remote run also passed 188 Chromium interaction/accessibility tests, both Lighthouse URL budgets (three runs each), and all 18 curated-preview checks. Live/golden fixtures are separately enabled below rather than treating conditional skips as evidence.
...
- Protected-main delivery: commit `bc6296deb4cea42c004cecda504cc3d8b18939b8` passed [all nine exact-head quality jobs](https://github.com/dajiaohuang/solar/actions/runs/35717969603), including Linux Go race/vulnerability checks, Android build/lint, iOS simulator build, both native real-SPK HTTPS scenarios, and all four browser projects. It then fast-forwarded main through the existing required status with no new PR or rule bypass. The simulator/emulator evidence does not establish physical-device performance or production deployment. This documentation-only follow-up uses repository checks without repeating the full application suites.
