...
Every build publishes `product-availability.json` and `ephemeris-manifest.json`. `build-info.json` records the product and availability hash. Runtime workers use the same selected manifest; packing recursively includes required source-specific dependencies and verifies exact SPK hashes and bytes. The current default preview closure contains 36 files totaling 90,732,544 SPK bytes; this is an artifact count, not a count of bodies with complete states. / 每次构建发布产品可用性与星历清单，构建信息记录产品及可用性哈希。运行时 worker 使用相同清单；打包递归包含各来源所需依赖并验证 SPK 哈希与字节数。当前默认预览闭包有 36 个文件、90,732,544 字节 SPK；这是产物数，不是状态完整的天体数量。
