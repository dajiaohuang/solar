...
| `npm run native:check` | Native project static check / 原生项目静态检查 | Checks native project structure only; no Web shell or SPK packaging / 仅检查原生项目结构，不构建 Web 壳或打包 SPK |
...
Every build publishes `product-availability.json` and `ephemeris-manifest.json`. `build-info.json` records the product and availability hash. Runtime workers use the same selected manifest; packing recursively includes required source-specific dependencies and verifies exact SPK hashes and bytes. The preview closure contains 41 files totaling 92,071,936 SPK bytes after adding the four SAT456 identities (NAIF 619–622); this is an artifact count, not a count of bodies with complete states. / 每次构建发布产品与星历清单，构建信息记录产品及可用性哈希。运行时 worker 使用相同清单；打包递归包含各来源所需依赖并验证 SPK 哈希与字节数。加入四个 SAT456 身份（NAIF 619–622）后，预览闭包有 41 个文件、92,071,936 字节 SPK；这是产物数，不是状态完整的天体数量。
...
The Pages workflow validates the Web source, then builds and tests the curated preview, applies Lighthouse/capacity gates and publishes only that preview. The preview is a Web product profile, not the native backend. Native source projects are independent first-slice prototypes using the state-tile protocol; Android has local real-SPK HTTPS emulator UI/cache evidence for its GLES point renderer (see MOBILE.md), and iOS has passed macOS CI protocol/build checks and real-SPK HTTPS simulator smoke tests (physical-device and full-feature validation remain pending; see MOBILE.md). Their manifest/plan loading remains online and no public full-native backend or complete all-body coverage is claimed. Deployment smoke verifies product identity, curated delivery checksums, permitted samples and blocked full-only actions. Only a successful deployment and live check prove publication. / Pages 流程验证 Web 源码，再构建并测试精选预览，通过 Lighthouse/容量门禁后仅发布该预览。预览是 Web 产品配置，不是原生后端。原生源码项目是使用状态瓦片协议的独立第一竖切原型；Android 的 GLES 点渲染器已有本地真实 SPK HTTPS 模拟器交互/缓存证据（见 MOBILE-CN.md），iOS 已通过 macOS CI 协议/构建及真实 SPK HTTPS 模拟器冒烟（真机与完整功能仍待验证，证据见 MOBILE-CN.md）。manifest/plan 仍需在线，不声称公开的原生完整版后端或完整全天体覆盖。线上冒烟验证产品身份、精选交付校验、允许样本和受限操作拦截；只有部署成功且线上验证通过才算发布。
