...
- At UTC JD 2461287.5, the integration test resolves **514 selectable centers**. Two registry entries lack SPK states: Makemake and S/2009 S1. Makemake's Horizons fixture does not establish a resolved primary center; S/2009 S1 has no corroborated target number. Ten added asteroid snapshots are frozen Horizons type-21 responses for Ida, Eros, Gaspra, Itokawa, Apophis, Ryugu, Phaethon, Florence, Didymos and Psyche; Bennu remains unavailable because Horizons explicitly reports that pre-computed SPK creation is not available for it. Daphnis uses the original historical SAT393 Type 17 record. Nine other Saturn moons use the distinct modern NAIF SAT415 delivery with its original DE437 dependencies, not the historical SSD container. These are explicit source and coverage boundaries, not absent observed objects.
...
The frozen asteroid extension is reproducible with `scripts/integrate-horizons-asteroid-pack-next.mjs` and the retained local cache selected by `SOLAR_HORIZONS_ASTEROID_NEXT_CACHE`. Its four targets (3200 Phaethon, 3122 Florence, 65803 Didymos and 16 Psyche) are direct NASA/JPL Horizons SPK responses, hashed at both the raw response and binary levels, then cropped only to the Pages 2026/2027 and full 2020/2031 TDB windows. The integration keeps original type-21 records and does not refit, extrapolate or apply an N-body model. The cache is release evidence and is not required at application startup.
