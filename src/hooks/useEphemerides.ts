...
export function useEphemerisLoading() {
  useEffect(() => {
    ...
    const load = () => {
      ...
      const selected = new Set([...selection.selectedIds, simulation.referenceId, simulation.comparisonReferenceId])
    ...
    load()
    const unsubscribe = [selectionStore.subscribe(load), simulationStore.subscribe(load), uiStore.subscribe(load)]
