...
export function projectGaiaDisplay(chunk: GaiaChunk, centerRaDeg: number, centerDecDeg: number, radiusDeg: number) {
  if (![centerRaDeg, centerDecDeg, radiusDeg].every(Number.isFinite) || Math.abs(centerDecDeg) > 90 || radiusDeg <= 0 || radiusDeg > 2) throw new Error('Invalid Gaia tangent view')
