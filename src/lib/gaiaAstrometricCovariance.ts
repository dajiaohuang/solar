...
const fields = ['ra', 'dec', 'parallax', 'pmra', 'pmdec'] as const

/** Formal five-coordinate marginal at the catalog epoch. No zero-point,
 * systematic, pseudocolour conditioning or epoch propagation is applied. */
export function gaiaAstrometricCovariance(source: GaiaSource) {
  ...
  const correlation: number[][] = Array.from({ length: 5 }, (_, i) => Array.from({ length: 5 }, (_, j) => i === j ? 1 : 0))
  ...
  const lower = Array.from({ length: 5 }, () => Array<number>(5).fill(0))
  ...
  for (let i = 0; i < 5; i++) for (let j = i; j < 5; j++) matrix[i][j] = matrix[j][i] = correlation[i][j]*sigmas[i]*sigmas[j]
  ...
  return { available: true as const, matrix, correlation, epochJulianYear: 2016, timeScale: 'TCB', frame: 'ICRS',
    coordinateLabels: ['delta-alpha*cos(delta)', 'delta-dec', 'parallax', 'pmra', 'pmdec'],
    coordinateUnits: ['mas', 'mas', 'mas', 'mas/Julian-year', 'mas/Julian-year'],
    matrixUnits: 'product-of-row-and-column-coordinate-units',
    sourceSolutionParameters: source.astrometric_params_solved === 95 ? 6 : 5,
    scope: 'five-astrometric-parameter-marginal', pseudocolourIncluded: false,
