...
export function ObservationReadout({ body, observer, julianDay }: { body: CelestialBody; observer: CelestialBody; julianDay: number }) {
  ...
  return <details className="model-note">
    ...
    {result ? <p>{t('distance')}: {Math.hypot(...result.position).toLocaleString(language, { maximumFractionDigits: 1 })} km<br />{language === 'zh' ? '光时' : 'Light time'}: {result.lightTimeSeconds.toFixed(4)} s</p> : <p>{language === 'zh' ? '当前目标、观测者或发射时刻无完整星历覆盖。' : 'Target, observer or emission epoch lacks complete ephemeris coverage.'}</p>}
