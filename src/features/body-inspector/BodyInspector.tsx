...
export function BodyInspector({ body, currentPositions, bodiesById }: Props) {
  ...
  const satelliteSourceFrame = satelliteEvidence?.sourceFrame === 'jpl-ecliptic' ? t('jplEclipticFrame') : t('undocumentedIllustrativeFrame')
  const satelliteSourceCenter = satelliteEvidence?.sourceCenter === 'earth-geocenter' ? t('earthGeocenter') : t('undocumentedParentCenter')
  const satelliteAppliedCenter = satelliteEvidence?.appliedCenter === 'earth-moon-barycenter-seed' ? t('earthMoonBarycenterSeed') : t('parentRenderedPoint')
  const satelliteCenterHandling = satelliteEvidence?.centerHandling === 'untransformed-center-offset' ? t('untransformedCenterOffset') : t('directParentAddition')
  const satellitePhaseProvenance = satelliteEvidence?.phaseProvenance === 'jpl-mean-elements' ? t('jplMeanElementsPhase') : t('illustrativeZeroPhase')
  const satellitePrecision = satelliteEvidence?.precision === 'fixed-mean-ellipse-not-ephemeris' ? t('fixedMeanEllipseBoundary') : t('illustrativeFixedEllipseBoundary')
