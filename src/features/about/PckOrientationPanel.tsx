...
export function PckOrientationPanel() {
  ...
  return <section className="evidence-module glass-panel" aria-label={zh ? '天体姿态' : 'Body orientation'}>
    ...
    <label className="field"><span><input type="checkbox" checked={includeLimb} onChange={event => { clear(); setIncludeLimb(event.target.checked) }} /> {zh ? '计算有限距离椭球轮廓' : 'Include finite-distance ellipsoid limb'}</span></label>
