...
export function GaiaPlot({ capacity, batches, zoom, onUploaded, onError, onSelect }: {
  ...
  const state = useRef<{ gl: WebGL2RenderingContext; buffer: WebGLBuffer; program: WebGLProgram; count: number; processed: number; draw: () => void } | null>(null)
  const latest = useRef({ zoom, onUploaded, onError }); latest.current = { zoom, onUploaded, onError }
  useEffect(() => {
    const element = canvas.current!, gl = element.getContext('webgl2', { alpha: false, antialias: false })
    ...
    const lost = (event: Event) => { event.preventDefault(); latest.current.onError('Gaia graphics context lost; reload the data.') }
    try {
      ...
      gl.useProgram(program); buffer = gl.createBuffer()!; gl.bindBuffer(gl.ARRAY_BUFFER, buffer); gl.bufferData(gl.ARRAY_BUFFER, Math.max(1,capacity)*12, gl.DYNAMIC_DRAW)
      ...
      const draw = () => {
        const width = Math.max(1,Math.round(element.clientWidth*Math.min(devicePixelRatio,2)))
        ...
        gl.uniform1f(zoomLocation,latest.current.zoom); gl.uniform1f(densityLocation,Math.min(devicePixelRatio,2)); gl.drawArrays(gl.POINTS,0,state.current?.count ?? 0)
      }
      state.current = { gl, buffer, program, count: 0, processed: 0, draw }
    ...
    return () => { observer?.disconnect(); element.removeEventListener('webglcontextlost',lost); state.current = null; if (buffer) gl.deleteBuffer(buffer); if (program) gl.deleteProgram(program); shaders.forEach(s => gl.deleteShader(s)) }
  },[capacity])
  useEffect(() => {
    const s = state.current; if (!s) return
    for (; s.processed < batches.length; s.processed++) {
      const batch = batches[s.processed]
      if (batch.display.length%3 || s.count+batch.display.length/3 > capacity) { onError('Gaia upload exceeds display capacity'); return }
      s.gl.bindBuffer(s.gl.ARRAY_BUFFER,s.buffer); s.gl.bufferSubData(s.gl.ARRAY_BUFFER,s.count*12,batch.display); s.count += batch.display.length/3
      s.draw(); onUploaded(batch.sequence)
    }
  ...
  return <canvas ref={canvas} role="img" aria-label="Gaia ICRS tangent-plane star chart at J2016.0" style={{ width:'100%', aspectRatio:'1', display:'block' }} onClick={event => {
    const rect = event.currentTarget.getBoundingClientRect(), x = ((event.clientX-rect.left)/rect.width*2-1)/zoom, y = (1-(event.clientY-rect.top)/rect.height*2)/zoom
    let index = 0, best = -1, distance = (24/rect.width/zoom)**2
    for (const batch of batches) for (let i = 0; i < batch.display.length; i += 3, index++) {
      const d = (batch.display[i]-x)**2+(batch.display[i+1]-y)**2
