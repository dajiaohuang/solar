...
test('packed historical trails survive switching the active renderer without a second canvas', async ({ page }, info) => {
  ...
  expect(historicalRequests).toHaveLength(1)
  expect(historicalRequests[0]).toContain('/state/window')
...
test('backend historical failures never invoke browser trajectories or erase verified current states', async ({ page }) => {
  ...
  page.on('pageerror', error => errors.push(error.message))
  await page.addInitScript(() => localStorage.setItem('solar-atlas-first-run-v1', 'complete'))
  await page.route('**/solar-test-api/v1/state/window*', route => route.fulfill({ status: 503, body: 'unavailable' }))
  ...
  await expect(page.locator('.canvas-error').filter({ hasText: 'State window HTTP/protocol failure 503' })).toBeVisible()
...
test('computes the Earth-to-Mars transfer and worker porkchop map', async ({ page }) => {
  ...
  await expect(page.locator('.metric > span').filter({ hasText: /^C3$/ })).toBeVisible()
...
test('searches workspaces, objects, stories, and terms from the keyboard palette', async ({ page }) => {
  ...
  await searchbox.dispatchEvent('keydown', { key: 'Enter', code: 'Enter', isComposing: true })
  await expect(dialog).toBeVisible()
  await searchbox.dispatchEvent('keydown', { key: 'Escape', code: 'Escape', isComposing: true })
  await expect(dialog).toBeVisible()
  await searchbox.press('ArrowDown')
  await expect(searchbox).toBeFocused()
  await expect(dialog.getByRole('option', { name: /The gaps carved by Jupiter/ })).toHaveAttribute('aria-selected', 'true')
  ...
  await page.getByRole('dialog', { name: 'Search Solar Atlas' }).getByRole('combobox').fill('Mars')
