...
test('keeps source-backed fallback identities selectable with an explicit diagnostic model', async ({ page }) => {
  ...
  await page.goto('?v=4&lang=en&ref=jupiter&bodies=jupiter,naif:510&focused=naif:510&jd=2466154.5&history=1')
  ...
  await expect(page.getByTestId('satellite-identity')).toContainText('NAIF 510')
...
test('does not draw a misleading origin when a fallback satellite is used as reference', async ({ page }) => {
  ...
  page.on('pageerror', error => errors.push(error.message))
  await page.addInitScript(() => localStorage.setItem('solar-atlas-first-run-v1', 'complete'))
  await page.goto('?v=4&lang=en&ref=naif:510&bodies=jupiter,naif:510&jd=2466154.5&history=1')
