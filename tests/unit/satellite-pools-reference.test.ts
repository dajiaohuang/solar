...
describe('integrated satellite source pools and delivery profiles', () => {
  ...
  it('keeps the same target identities with explicit narrower Pages windows', () => {
    ...
    expect(full.files.reduce((total, file) => total + file.bytes, 0)).toBe(1181799424)
    expect(pages.files.reduce((total, file) => total + file.bytes, 0)).toBe(274401280)
  })
  ...
  it('pins the bounded Horizons asteroid snapshots without treating the API as immutable', () => {
    const expected = new Map([
      ...
      [20000002, '2'], [20000004, '4'], [20000010, '10'], [20000016, '16'],
      [20015810, '15810'], [20090377, '90377'], [20028978, '28978'], [20225088, '225088'], [20174567, '174567'],
      [20136472, '136472'], [20055637, '55637'], [20084522, '84522'], [20145451, '145451'], [20145452, '145452'],
      [20055565, '55565'], [20307261, '307261'], [20208996, '208996'], [20120132, '120132'], [20055636, '55636'],
  ...
  it('evaluates every batch-18 Horizons root at both delivery profiles and records the SB441 upgrade boundary', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-asteroids-batch18-20260910')
    expect(roots).toHaveLength(4)
    expect(roots.map(root => root.targets[0])).toEqual([20000002, 20000004, 20000010, 20000016])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
    ...
    expect(full.files.some(file => file.id.startsWith('sb441-n16-2000002'))).toBe(false)
    expect(full.files.some(file => file.id.startsWith('sb441-n16-2000004'))).toBe(false)
    expect(full.files.some(file => file.id.startsWith('sb441-n16-2000010'))).toBe(false)
    expect(full.files.some(file => file.id.startsWith('sb441-n16-2000016'))).toBe(false)
  })

  it('evaluates every batch-19 Horizons TNO root at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-asteroids-batch19-20260914')
    expect(roots).toHaveLength(5)
    expect(roots.map(root => root.targets[0])).toEqual([20090377, 20015810, 20028978, 20225088, 20174567])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      expect(root.selectionEvidence.stateBoundary).toBeUndefined()
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
  ...
  it('evaluates every batch-20 Horizons TNO root at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-asteroids-batch20-20260914')
    expect(roots).toHaveLength(5)
    expect(roots.map(root => root.targets[0])).toEqual([20136472, 20055637, 20084522, 20145451, 20145452])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      expect(root.selectionEvidence.stateBoundary).toBeUndefined()
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
  ...
  it('evaluates every batch-21 Horizons TNO root at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-asteroids-batch21-20260914')
    expect(roots).toHaveLength(5)
    expect(roots.map(root => root.targets[0])).toEqual([20055565, 20307261, 20208996, 20120132, 20055636])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      expect(root.selectionEvidence.stateBoundary).toBeUndefined()
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
  ...
  it('evaluates every batch-22 Horizons TNO root at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-asteroids-batch22-20260914')
    expect(roots).toHaveLength(5)
    expect(roots.map(root => root.targets[0])).toEqual([20015760, 20019521, 20038083, 20038628, 20053311])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      expect(root.selectionEvidence.stateBoundary).toBeUndefined()
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
  ...
  it('evaluates every batch-23 Horizons TNO root at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-asteroids-batch23-20260914')
    expect(roots).toHaveLength(8)
    expect(roots.map(root => root.targets[0])).toEqual([20523794, 50666516, 50760674, 20532037, 20541132, 50773852, 20612911, 20471143])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      expect(root.selectionEvidence.stateBoundary).toBeUndefined()
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
  ...
  it('evaluates every batch-24 Horizons TNO root at both delivery profiles and preserves primary-only boundaries', () => {
  ...
  it('evaluates every batch-27 Horizons Centaur root at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-centaur-batch27-20260914')
    expect(roots).toHaveLength(3)
    expect(roots.map(root => root.targets[0])).toEqual([20005335, 20031824, 20032532])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      expect(root.selectionEvidence.stateBoundary).toBeUndefined()
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
  ...
  it('evaluates every batch-28 Horizons primary root at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-centaur-batch28-20260914')
    expect(roots).toHaveLength(3)
    expect(roots.map(root => root.targets[0])).toEqual([20029981, 20052872, 20052975])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      expect(root.selectionEvidence.stateBoundary).toBeUndefined()
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
  ...
  it('evaluates the batch-29 Horizons primary root at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-centaur-batch29-20260914')
    expect(roots).toHaveLength(1)
    expect(roots.map(root => root.targets[0])).toEqual([20055576])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
      ...
      const kernel = new SpkKernel(bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength))
      const target = root.targets[0]
      for (const et of [root.startEt, (root.startEt + root.endEt) / 2, root.endEt]) expect(kernel.evaluate(target, et), `${root.id}/${et}`).not.toBeNull()
    }
  })

  it('evaluates the batch-30 Horizons primary roots at both delivery profiles and preserves primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-centaur-batch30-20260914')
    expect(roots).toHaveLength(3)
    expect(roots.map(root => root.targets[0])).toEqual([20037117, 20083982, 20346889])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
  ...
  it('evaluates the batch-31 Horizons primary roots at both delivery profiles and preserves Typhon primary-only boundaries', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-centaur-batch31-20260914')
    expect(roots).toHaveLength(3)
    expect(roots.map(root => root.targets[0])).toEqual([20042355, 20330836, 20463368])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
  ...
  it('evaluates the batch-32 Horizons primary roots at both delivery profiles', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-centaur-batch32-20260914')
    expect(roots).toHaveLength(3)
    expect(roots.map(root => root.targets[0])).toEqual([20696315, 20078799, 20090568])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
  ...
  it('evaluates the batch-33 Horizons TNO roots at both delivery profiles', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-tno-batch33-20260914')
    expect(roots).toHaveLength(3)
    expect(roots.map(root => root.targets[0])).toEqual([20058534, 20065489, 20066652])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
  ...
  it('evaluates the batch-34 Horizons TNO roots at both delivery profiles', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-tno-batch34-20260914')
    expect(roots).toHaveLength(2)
    expect(roots.map(root => root.targets[0])).toEqual([20088611, 20341520])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
  ...
  it('evaluates the batch-35 Horizons TNO roots at both delivery profiles', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-tno-batch35-20260914')
    expect(roots).toHaveLength(3)
    expect(roots.map(root => root.targets[0])).toEqual([20015789, 20127546, 20762135])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
  ...
  it('evaluates the batch-36 Horizons TNO roots at both delivery profiles', () => {
    const roots = full.files.filter(file => file.integrationBatch === 'horizons-tno-batch36-20260915')
    expect(roots).toHaveLength(2)
    expect(roots.map(root => root.targets[0])).toEqual([20148780, 20184314])
    for (const root of roots) {
      expect(root.targets).toHaveLength(1)
      const bytes = readFileSync(`public/data/ephemerides/${root.path}`)
      expect(bytes.length).toBe(root.bytes)
