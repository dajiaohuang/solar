...
describe('reference-frame view radius', () => {
  it('includes source-backed satellite fallback seeds in the local framing extent', () => {
    const aoede = majorBodiesById.get('naif:541')!
    expect(aoede.source).toBe('jpl-spk-osculating-fallback')
    expect(aoede.orbit?.model).toBe('keplerian')
    const orbit = aoede.orbit!
    if (orbit.model !== 'keplerian') throw new Error('Expected a generated Keplerian Aoede fallback seed')
    const expected = Math.max(.08, orbit.semiMajorAxisAU * (1 + orbit.eccentricity)) * 1.18
    expect(getSuggestedViewRadius(['jupiter', 'naif:541'], 'jupiter', majorBodiesById, .08)).toBeCloseTo(expected, 12)
    expect(getSuggestedViewRadius(['jupiter', 'naif:541'], 'jupiter', majorBodiesById, NaN)).toBeGreaterThan(0)
  })
