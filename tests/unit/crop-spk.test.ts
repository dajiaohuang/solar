...
function syntheticSource() {
  ...
  ;[499, 10, 1, 2, 385, 412].forEach((n, i) => v.setInt32(summary + 40 + i * 4, n, true))
  const at = (address: number) => (address - 1) * 8
  for (let record = 0; record < 2; record++) {
    const offset = at(385 + record * 12)
  ...
  const meta = at(409); v.setFloat64(meta, 0, true); v.setFloat64(meta + 8, 10, true); v.setFloat64(meta + 16, 12, true); v.setFloat64(meta + 24, 2, true)
...
describe('crop-spk', () => {
  it('retains complete original records and emits a readable DAF/SPK', async () => {
    ...
    const kernel = new SpkKernel(result.buffer)
    expect(kernel.segments[0]).toMatchObject({ target: 499, startEt: 5, endEt: 15, recordCount: 2, recordSize: 12 })
