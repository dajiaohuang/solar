...
test('preserves sample identity, invalid offsets and failures without partial cancellation results', async () => {
  ...
  expect(result.finalStates.slice(0,6)).toEqual(result.initial.states.slice(0,6))
  expect(result.finalStates.slice(12,18)).toEqual(result.initial.states.slice(12,18))
  expect(result.numerics).toEqual([{ index: 0,evaluations: 0,accepted: 0,rejected: 0 },{ index: 2,evaluations: 0,accepted: 0,rejected: 0 }])
