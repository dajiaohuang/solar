...
两端原生客户端现在提供相同的九个有来源依据的预设：行星、地月、火星卫星、木星卫星、土星卫星、天王星卫星、海王星卫星、冥王星卫星，以及选定的 SPK 小行星。预设 ID 只是请求选择；精确或缺失的可用性仍由配置的后端及其来源/模型证据决定。
...
过期选择用例与真实 SPK 状态验证分开。完整三端功能一致性尚未完成；iOS 来源浏览
实现见下文。
...
iOS 新增独立、默认收起的原生来源列表；展开不会请求数据。加载第一页或下一页时，
先获取新的 HTTPS 清单（最多 8 MiB），再获取最多 50 条来源记录（256 KiB），只保留
一页。查询上限为 256 个 UTF-8 字节。展示原始来源 ID、行号、身份声明及星历状态，
明确提示来源记录不等于独立天体或已验证状态。

选入本页会替换自定义 ID 并收起列表，不计算状态。加载观测时补入参考 ID，并在
请求状态计划前，以新的清单核对所选页的后端、星历目录和来源目录身份；变化即拒绝。
编辑自定义 ID 会解除所选页绑定；修改历元或参考 ID 保留绑定，但清除旧观测。
修改后端还会清除绑定和目录结果。修改查询、收起或进入后台会取消目录请求，
清空显示页，迟到响应不能重新发布旧结果。每次 HTTP 传输有大小上限、拒绝重定向，
采用 30 秒请求/120 秒资源超时；这不是整个两次请求操作的硬截止时间。
...
在 `77c2a8a` 的
[CI 33986754016](https://github.com/dajiaohuang/solar/actions/runs/33986754016)
中，两次 50 个 ID 断言及收起/重新展开均通过，但最后的加载没有到达服务器：
截图和元素坐标显示按钮仍在导航栏后，尽管 XCTest 判定可点击。滚动辅助方法现要求
目标完整进入内容可视区，并保留点击前截图。`157121d` 的
[CI 33987782740](https://github.com/dajiaohuang/solar/actions/runs/33987782740)
已通过 iOS 协议、构建及全部 5 项模拟器测试。目录测试保留全部 50 个选中 ID，
收起/重开后保持一致，并在任何状态计划前拒绝清单变化；5 次 GET 账本与已检查的
点击前/拒绝后截图一致。这是合成目录用例；独立的地月状态及校验缓存测试使用真实
SPK。该轮 Web 作业因 32K 单元测试超时失败，不能称为整轮 CI 全绿。
