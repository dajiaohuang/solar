...
两端原生客户端现在提供相同的九个有来源依据的预设：行星、地月、火星卫星、木星卫星、土星卫星、天王星卫星、海王星卫星、冥王星卫星，以及完整打包的 96 个 SPK 小行星身份（92 个 Horizons 快照和 4 个 SB441 来源）。预设 ID 只是请求选择；精确或缺失的可用性仍由配置的后端及其来源/模型证据决定。
