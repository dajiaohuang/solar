...
func classifyRequest(r *http.Request) requestClass {
	...
	if r.Method == http.MethodPost && (r.URL.Path == "/v1/trajectory" || r.URL.Path == "/v1/state/window" || r.URL.Path == "/v1/observation/windows" || r.URL.Path == "/v1/observation/contacts" || r.URL.Path == "/v1/stellar/motion") {
		return trajectoryWork
	}
