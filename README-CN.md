...
Solar Atlas 当前提供二十二个一键预设，包括围绕八个母天体的九组卫星目录场景，以及 16 颗大型小行星场景。每个预设都固定历元、参考系、聚焦集合、视图、缩放和轨迹窗口；数据集驱动预设还会固定完整的数据版本、样本和筛选组合。

除地球的月球外，冻结的卫星目录包含 464 个身份（460 条发现目录记录、1 个额外行星卫星身份以及 3 个海王星外天体伴星）。其中 463 个已核对 SPK 目标编号，S/2009 S1 仍未匹配。收录身份**不代表**已有位置、物理半径或正式发现确认；新增条目没有编造的椭圆回退。当前位置缺失与历史轨迹缺口分别提示，参考天体无状态时不显示替代原点。土星的 293 个目录身份分为两组预设，遵守 3D 的 160 个聚焦对象上限。详见[可重放的卫星证据流程](./scripts/reference/SATELLITE-SURVEY.md)。
...
按需加载、SHA-256 固定的 SPK 包现有两种配置，各含 502 个文件：Pages 为 **235.8 MiB**（247,241,728 字节），完整／原生包为 **647.4 MiB**（678,845,440 字节）。在 UTC JD 2461287.5，测试确认 **492 个可选天体中心**有 SPK 状态。完整包新增卫星覆盖 2020–2031 TDB；Pages 保留相同目标身份，将较大的卫星文件缩短为 2026–2027。原始 type 2/3/17/21 记录明确绑定各自的来源中心依赖；Daphnis 保留历史 SAT393／内嵌 DE431 组合，九颗新增土星卫星使用现代 NAIF SAT415／内嵌 DE437。独立 CSPICE 对照检查了 436 组新增来源组合、1,308 个时刻；数值一致不等于物理不确定度已评估。
