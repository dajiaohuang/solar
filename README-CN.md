...
- **访客层：** 双语首次访问首页、三条清晰起点、全局天体/故事/术语搜索（`Ctrl/⌘ K` 或 `/`）、浏览器前进/后退、描述性页面标题，以及四项移动端主导航。
- **综合观测台：** 八个一键预设场景；常驻二维/三维切换；有界时间模拟；任意天体中心和分屏参考系；可搜索的渲染天体选择；轨迹、图层、缩放与视角参数；测距、拉格朗日点、希尔球与拉普拉斯影响球。
