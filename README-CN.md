...
Solar Atlas 当前提供二十个一键预设，包括围绕六个母天体的七组卫星目录场景，以及 16 颗大型小行星场景。每个预设都固定历元、参考系、聚焦集合、视图、缩放和轨迹窗口；数据集驱动预设还会固定完整的数据版本、样本和筛选组合。

除地球的月球外，冻结的卫星目录包含 461 个身份（460 条发现目录记录和 1 个原始星历中明确列名的额外目标）。其中 460 个已核对 SPK 目标编号，S/2009 S1 仍未匹配。收录身份**不代表**已有位置、物理半径或正式发现确认；新增条目没有编造的椭圆回退。当前位置缺失与历史轨迹缺口分别提示，参考天体无状态时不显示替代原点。土星的 293 个目录身份分为两组预设，遵守 3D 的 160 个聚焦对象上限。详见[可重放的卫星证据流程](./scripts/reference/SATELLITE-SURVEY.md)。
...
| SPK 星历 | 原始 NAIF/JPL SPK type 2/3/17/21 记录，沿各来源的 ECLIPJ2000 中心链计算几何天体中心状态。`de440s`：2000–2051；完整卫星/小行星内核：2020–2031；Pages 较大内卫星文件：2026–2027；Eris/Haumea 主星中心止于 2030-01-02 TDB。不重拟合、不重采样、不外推，不重复施加力模型修正 |
...
按需加载、SHA-256 固定的 SPK 包现有两种配置，各含 487 个文件：Pages 为 **207.5 MiB**（217,626,624 字节），完整／原生包为 **500.2 MiB**（524,467,200 字节）。在 UTC JD 2461287.5，测试确认 **479 个可选天体中心**有 SPK 状态。完整包新增卫星覆盖 2020–2031 TDB；Pages 保留相同目标身份，将较大的内卫星文件缩短为 2026–2027。原始 type 2/3/17/21 记录明确绑定各自的 DE440／DE441／DE442 中心依赖。独立 CSPICE 对照检查了 423 组新增来源组合、1,269 个时刻；数值一致不等于物理不确定度已评估。

这仍不是全部覆盖：十颗已有编号的土星卫星与 S/2009 S1 在本包中仍缺现代 SPK 状态。Makemake 的 Horizons 轨道解不等于已确认主星中心，故保留近似回退；Eris／Haumea 使用原发布主星偏移，止于 2030-01-02 TDB。缺少状态不会编造轨道。普通构建不下载内核：网页默认 Pages，原生默认完整包；`SOLAR_ATLAS_EPHEMERIS_PROFILE=full` 可选择完整网页发行。原生 SPK 离线可用，目录与实时 SBDB 查询保持联网边界。旧 `data:ephemerides` 命令不是完整卫星刷新流程，详见[配置与再生成契约](./docs/physical-ephemerides.md)和[卫星证据流程](./scripts/reference/SATELLITE-SURVEY.md)。
