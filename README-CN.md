...
预览配置新增 SAT456 身份 NAIF 619–622；下一次精选闭包为 41 个 SPK 文件、共 92,071,936 字节。部署前，线上 Pages 仍是此前的 36 文件快照。

SHA-256 固定、按需加载的 Web/后端 **来源配置**有两种，各含 612 个文件：Pages 配置为 **260.5 MiB**（273,137,664 字节），完整版 Web/后端为 **1115.7 MiB**（1,169,903,616 字节）。当前线上 Pages 预览是另一份精选闭包，仅包含 36 个 SPK 文件、90,687,488 字节；线上容量报告总计 93.2 MiB，不能与 612 文件的 Pages 来源配置混淆。在 UTC JD 2461287.5，来源集成测试确认 **599 个可选天体中心**有 SPK 状态；这不代表原生覆盖或完整离线星历。完整行星卫星新增覆盖 2020–2031 TDB；Pages 将大型卫星文件缩短为 2026–2027。八组新增小天体双星以及一百零六份固定 Horizons type-21 小行星快照（Pallas、Vesta、Hygiea、Psyche、Ida、Eros、Gaspra、Itokawa、Apophis、Ryugu、Phaethon、Florence、Didymos、Toutatis、Ganymed、Betulia、Steins、1998 OR2、1950 DA、2001 FO32、Arrokoth、APL、Dinkinesh、2008 EV5、Kamoʻoalewa、2000 ET70、2001 SN263、2005 YU55、2003 SD220、2004 BL86、Duende、Hebe、Metis、Irene、Melpomene、Fortuna、Antiope、Kleopatra、Parthenope、Egeria、Lutetia、Themis、Amphitrite、Laetitia、Nysa、Astraea、Flora、Victoria、Massalia、Harmonia、Cruithne、Golevka、Besixdouze、Torifune、Kalliope、Eugenia、Minerva、Hermione、Elektra、Juno、Iris、Euphrosyne、Europa、Cybele、Phocaea、Euterpe、Urania、Circe、Fides、Sylvia、Camilla、Davida、Interamnia、Thetis、Thalia、Proserpina、Bellona、Pomona、Nemausa、Chiron、Pholus、Chariklo、Varuna、Eunomia、Thisbe、Hektor、Agamemnon、Polyhymnia、Leukothea、Atalante、Leda、Daphne、Hestia、Doris、Pales）已纳入；后者完整配置覆盖 2020–2031、Pages 覆盖 2026–2027，并保留响应与二进制哈希。新增的十四个外太阳系天体（Chiron、Pholus、Chariklo、Varuna、Sedna、Arawn、Ixion、Gonggong、Varda、Makemake、2002 UX25、2002 TC302、2005 RM43、2005 RN43）仅提供直接目标中心状态；其 Centaur/TNO 标签只是目录上下文，不代表共享动力学拟合、伴星或实测物性。Eunomia 与 Thisbe 使用直接 Horizons 身份，替代通用 SB441 目标拼写。所有冻结 Horizons 条目均仅提供直接主天体日心状态；不交付伴星或系统中心状态，也不暗示双星覆盖。Bennu 仍明确属于 Horizons 生成缺口。原始 type 2/3/17/21 记录明确绑定各自的来源中心依赖；独立 CSPICE 检查 444 组新增来源池和 1,422 组位置/速度样本；数值一致不等于物理不确定度已评估。

这仍不是全部覆盖：S/2009 S1 在来源包中没有可验证的 SPK 目标或状态；Daphnis 使用历史 SAT393 记录，精选 Makemake 条目仍保留近似回退，但 batch 20 提供了精确的 asteroid:136472 目标中心 SPK；Eris/Haumea 主星中心及其卫星止于 2030-01-02 TDB。缺少状态不会编造轨道。原生客户端不打包这些 SPK 配置，而是通过 manifest/plan/二进制状态瓦片获取精确当前位置；manifest 与 plan 仍需在线，已验证瓦片可从原生缓存复用。详见[配置与再生成契约](./docs/physical-ephemerides.md)和[卫星证据流程](./scripts/reference/SATELLITE-SURVEY.md)。
