...
| SPK 星历 | 原始 NAIF/JPL SPK type 2/3/21 记录，沿 ECLIPJ2000 中心链计算几何天体中心状态。`de440s`：2000–2051；已有卫星/小行星内核：2020–2031；Eris/Haumea 主星中心：2020-01-01–2030-01-02 TDB。不重拟合、不重采样、不外推，不重复施加力模型修正 |
...
物理星历包包含 61 个经 SHA-256 固定的文件（127,644,672 字节，约 121.7 MiB），按需加载。在现代测试历元，65 个可选天体中心有星历覆盖，包括使用原发布系统—主星偏移的 Eris 与 Haumea；两者止于 2030-01-02 TDB，不在无关场景加载。Makemake 仍明确使用近似回退：验证其 Horizons 轨道解不等于确认主星中心。目录还包含此前新增的 31 颗卫星和 15 颗小行星；这不代表全部天体均已覆盖。普通 `npm run build` 不下载内核，也不联网。网页与原生构建只打包 manifest 指定文件，原生 SPK 可离线使用；目录与实时 SBDB 查询仍保留联网边界。使用 `npm run data:ephemerides` 重新生成，或设置 `SOLAR_EPHEMERIS_FROM` / `SOLAR_EPHEMERIS_TO` 调整来源支持的本地/原生区间，详见[物理契约](./docs/physical-ephemerides.md)和[type 21 独立验证](./docs/spk21-validation.md)。
