...
除地球的月球外，冻结的卫星目录包含 472 个身份（460 条发现目录记录、1 个额外行星卫星身份以及 11 个海王星外天体伴星）。其中 471 个已核对 SPK 目标编号，S/2009 S1 仍未匹配。收录身份**不代表**已有位置、物理半径或正式发现确认；新增条目没有编造的椭圆回退。当前位置缺失与历史轨迹缺口分别提示，参考天体无状态时不显示替代原点。土星的 293 个目录身份分为两组预设，遵守 3D 的 160 个聚焦对象上限。详见[可重放的卫星证据流程](./scripts/reference/SATELLITE-SURVEY.md)。
...
常规 `build:deploy` 仍保留已安装客户端依赖的完整目录 URL。**在完整版数据端点与客户端迁移通过验证之前，不要用精选构建替换线上 Pages 产物。** 原生构建拒绝预览产品配置。准确的资源、缓存边界与验证命令见[预览交付与迁移说明](./docs/preview-delivery.md)。
