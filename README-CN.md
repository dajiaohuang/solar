...
按需加载、SHA-256 固定的 SPK 包现有两种配置，各含 499 个文件：Pages 为 **227.4 MiB**（238,471,168 字节），完整／原生包为 **563.8 MiB**（591,139,840 字节）。在 UTC JD 2461287.5，测试确认 **489 个可选天体中心**有 SPK 状态。完整包新增卫星覆盖 2020–2031 TDB；Pages 保留相同目标身份，将较大的内卫星文件缩短为 2026–2027。原始 type 2/3/17/21 记录明确绑定各自的来源中心依赖；Daphnis 保留历史 SAT393／内嵌 DE431 组合，九颗新增土星卫星使用现代 NAIF SAT415／内嵌 DE437。独立 CSPICE 对照检查了 433 组新增来源组合、1,299 个时刻；数值一致不等于物理不确定度已评估。
