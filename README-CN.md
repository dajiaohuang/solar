...
新增独立的身份与时间窗审计：在完整 SPK 配置下重新核对 1,567,193 条来源记录，得到 496 个明确映射的 NAIF 目标；对 2020–2031 TDB 请求窗，其中 486 个目标的描述符及中心依赖覆盖完整，10 个存在来源结束后的缺口。**这不是整窗数值精度认证，也不表示其余记录已获得轨道。** 审计输出保留来源序号、哈希、单历元六维状态及逐边界缺口，使用方法与限制见[来源清单文档](./docs/all-body-inventory.md#explicit-identity-and-dependency-window-ledger)。
