...
已配置后端的完整版 Web 在“证据与数据”中提供显式的“加载覆盖摘要”操作。它将报告与本次获取的后端目录／来源目录清单哈希匹配，分别展示来源记录与不同目标、审计历元、依赖时间窗和未解析原因。报告缺失、错配或验证失败时不显示总数；重试会清除旧报告。Pages 不请求此完整后端审计。摘要上限为 64 KiB，不下载全来源目录，也不认证整窗数值精度。原生端覆盖报告界面尚未实现。
