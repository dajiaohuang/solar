...
SHA-256 固定、按需加载的 Web/后端 **来源配置**有两种，各含 526 个文件：Pages 配置为 **258.7 MiB**（271,267,840 字节），完整版 Web/后端为 **1098.0 MiB**（1,151,318,016 字节）。当前线上 Pages 预览是另一份精选闭包，仅包含 36 个 SPK 文件、90,800,128 字节；线上容量报告总计 93.2 MiB，不能与 526 文件的 Pages 来源配置混淆。在 UTC JD 2461287.5，来源集成测试确认 **524 个可选天体中心**有 SPK 状态；这不代表原生覆盖或完整离线星历。完整行星卫星新增覆盖 2020–2031 TDB；Pages 将大型卫星文件缩短为 2026–2027。八组新增小天体双星以及十六份固定 Horizons type-21 小行星快照（Ida、Eros、Gaspra、Itokawa、Apophis、Ryugu、Phaethon、Florence、Didymos、Toutatis、Ganymed、Betulia、Steins、1998 OR2、1950 DA、2001 FO32）已纳入；后者完整配置覆盖 2020–2031、Pages 覆盖 2026–2027，并保留响应与二进制哈希。Bennu 仍明确属于 Horizons 生成缺口。原始 type 2/3/17/21 记录明确绑定各自的来源中心依赖；独立 CSPICE 检查 444 组新增来源池和 1,380 组位置/速度样本；数值一致不等于物理不确定度已评估。
