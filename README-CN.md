...
SPK 输出是声明参考系中的几何天体中心状态，并沿中心链解析；它不是客户端 N 体积分器，应用不会向来源轨迹再次叠加通用广义相对论或 J2 修正。焦点轨迹可使用 SPK，而 GPU 目录点云仍使用开普勒模型。几何、接收光时与恒星光行差读数彼此分开。配置好数据的完整版后端还提供[地面观测](./docs/ground-observation.md)：WGS84 站点、固定版本 IERS 与 SOFA 派生坐标转换、有限距离太阳光偏折及可选折射；这些修正作用于观测方向，不改写 SPK 轨迹。协方差传播和升落搜索仍待完成。
