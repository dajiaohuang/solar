...
- **All-source directory** — a collapsed Catalog panel queries the configured Go inventory in 50-record pages, preserving original source IDs, aliases and explicit assertions. An explicit TDB epoch check retrieves this page's verified Float64 states or missing reasons through the state-tile protocol. Source totals are not unique-body or exact-coverage totals; this inspection does not add records to the Observation Deck. Pages keeps the Catalog entry visible but blocks the full workspace with an explanation before any full-backend request.
...
3. **Trail/detail budget** — at most 160 requested historical trails in 3D or 320 in 2D. This does not truncate current-state requests or selection: verified states remain available, while a separate adaptive display budget reports the drawn and unshown counts. Extra 3D positions share fixed-pixel GPU points instead of individual meshes; object lists paginate without dropping selection. These points retain their actual SPK/fallback/missing-state classification and are not the approximate catalog cloud.
...
JPL SBDB values are read from the documented `orbit.elements[]` records (`name`, `value`, and `units`), not invented flat properties. The object-level `orbit.condition_code` is exposed as an orbit condition code. The Evidence page separately imports [source covariance](./docs/orbit-uncertainty.md), converts it at the solution epoch and displays projected contours while retaining additional model axes. Temporal full-model covariance propagation remains incomplete. Absolute-magnitude filters have explicit all/known/unknown states; an unknown H is never fabricated as a number and is excluded from numeric `a–H` plots.
...
The preview profile includes SAT456 identities NAIF 619–622; its curated closure is 41 SPK files totaling 92,071,936 bytes, matching the current Pages delivery contract.

The SHA-256-pinned, on-demand Web/backend **source profiles** now contain 670 files: the Pages profile is **261.7 MiB** (274,401,280 bytes), while full Web/backend is **1127.1 MiB** (1,181,799,424 bytes). The published Pages preview is a separate curated closure; source-profile totals must not be confused with deployed capacity. At UTC JD 2461287.5, source integration tests resolve **657 selectable body centers**; this is not a promise of native coverage or a complete offline ephemeris. Full planetary-satellite additions cover 2020–2031 TDB; Pages narrows large satellite files to 2026–2027. The source bundle includes 164 frozen Horizons type-21 asteroid snapshots, including batch-21 additions 2002 AW197 (55565), 2002 MS4 (307261), 2003 AZ84 (208996), 2004 GV9 (120132) and 2002 TX300 (55636), batch-22 additions Albion (15760), Chaos (19521), Rhadamanthus (38083), Huya (38628) and Deucalion (53311), batch-23 additions 523794 (2015 RR245), 2012 VP113, 2014 UZ224, 532037 Chiminigagua, 541132 Leleakuhonua, 2013 SY99, 612911 (2004 XR190) and 471143 Dziewanna, batch-24 additions 15874 (1996 TL66), 19308 (1996 TO66), 42301 (2001 UR163), 47171 Lempo (1999 TC36), 79360 Sila-Nunam (1997 CS29), 84922 (2003 VS2), 145453 (2005 RR43) and 229762 G!kun||'homdima (2007 UK126), batch-25 additions 7066 Nessus (1993 HA2), 8405 Asbolus (1995 GO) and 10370 Hylonome (1995 DW2), batch-26 additions 944 Hidalgo (A920 UB), 60558 Echeclus (2000 EC98) and 54598 Bienor (2000 QC243), and batch-27 additions 5335 Damocles (1991 DA), 31824 Elatus (1999 UG5) and 32532 Thereus (2001 PT13), and batch-28 additions 29981 (1999 TD10), 52872 Okyrhoe (1998 SG35) and 52975 Cyllarus, and batch-29 addition 55576 Amycus, batch-30 additions 37117 Narcissus, 83982 Crantor and 346889 Rhiphonos, and batch-31 additions 42355 Typhon, 330836 Orius and 463368 Eurytus, and batch-32 additions 696315 Petraios, 78799 Xewioso and 90568 Goibniu, and batch-33 additions 58534 Logos, 65489 Ceto and 66652 Borasisi, and batch-34 additions 88611 Teharonhiawako and 341520 Mors-Somnus. Batch 35 adds 15789 (1993 SB), 127546 (2002 XU93) and 762135 (2010 WG9), and batch 36 adds 148780 Altjira and 184314 Mbabamwanawaresa. All frozen Horizons entries are direct primary heliocentric states only; no companion or system-center state is delivered, and no binary coverage is implied. Bennu remains an explicit Horizons-generation gap. Original type 2/3/17/21 records retain explicit source-specific center dependencies; numerical agreement is not physical uncertainty.
...
SPK output is geometric, center-resolved state in its declared frame. The app does not add a second general-relativistic or J2 correction to source trajectories. Focus trajectories may use SPK states while the GPU catalog cloud remains Keplerian. Geometric, reception light-time, and stellar-aberration readouts are separate. A configured full backend also provides [ground observations](./docs/ground-observation.md) with WGS84 stations, pinned IERS/SOFA-derived transforms, finite-distance solar light deflection and optional refraction, plus bounded airless target-center rise/set and visibility searches. Observational corrections affect directions, not the SPK trajectory; search sampling limits and physical uncertainty remain explicit.

The Evidence page's separate [dynamics laboratory](./docs/dynamics-laboratory.md) integrates an explicitly adopted test-particle model driven by pinned DE440 masses. It offers optional solar 1PN corrections, variational equations, bounded trajectory projections, numerical refinement comparisons and source-bearing exports. Its model-integrated trajectories do not replace original SPK states or implement complete source-fit covariance propagation.
...
In a configured full Web deployment, Evidence offers an explicit **Load coverage summary** action. It checks the report against fresh backend catalog/inventory manifest hashes, keeps source rows separate from distinct targets, and shows the audit epoch, dependency window, source/model state buckets and unresolved reasons. A missing, mismatched or invalid report produces no totals; retrying clears the previous report. Pages never requests this full-backend audit. The summary is bounded at 64 KiB and does not download the complete source inventory or certify whole-window numerical accuracy. Android and iOS expose the same bounded report through their native coverage panels; native clients do not infer coverage when the report is unavailable.

A separate identity/window audit rechecks all 1,567,193 source rows against the full SPK profile: 507 mapped source rows represent 502 explicit NAIF targets with states at the audit epoch. Named small-body moons are reconciled through confirmed source names and explicit primary identities; five Pluto moon aliases do not inflate the target count. For the requested 2020–2031 TDB window, 486 targets have complete descriptor/center dependency availability and 16 have gaps. **This is not whole-window numerical-accuracy certification or orbital coverage for the remaining records.** The ledger retains source ordinals, mapping evidence, hashes, one-epoch six-vectors and boundary-specific gaps; see the [audit commands and limitations](./docs/all-body-inventory.md#explicit-identity-and-dependency-window-ledger).
...
Android and iOS are independent native projects, not a Web shell. Their source currently represents a first-slice prototype for exact current-state binary tiles from a user-selected HTTPS backend: Android has local real-SPK HTTPS emulator UI/cache evidence for its GLES point renderer (see MOBILE.md), and iOS has passed macOS CI protocol/build checks and real-SPK HTTPS simulator smoke tests (physical-device and full-feature validation remain pending; see MOBILE.md). The current iOS slice supports a TDB Julian date, presets or custom IDs, a reference ID, native 3D by default and native 2D, with a bounded 256 MiB verified-tile cache. Manifest and plan loading remains online; complete offline plan recovery is not implemented. No full-feature, real-device, production-signed binary or store-release claim is made; build evidence is linked in MOBILE.md. See [MOBILE.md](./MOBILE.md) and [PRIVACY.md](./PRIVACY.md).
