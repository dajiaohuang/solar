...
A separate identity/window audit now rechecks all 1,567,193 source rows against the full SPK profile and identifies 496 explicitly mapped NAIF targets. For the requested 2020–2031 TDB window, 486 of those targets have complete descriptor/center dependency availability and 10 have end-of-source gaps. **This is not whole-window numerical-accuracy certification or orbital coverage for the remaining records.** The ledger retains source ordinals, hashes, one-epoch six-vectors and boundary-specific gaps; see the [audit commands and limitations](./docs/all-body-inventory.md#explicit-identity-and-dependency-window-ledger).
