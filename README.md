...
In a configured full Web deployment, Evidence offers an explicit **Load coverage summary** action. It checks the report against fresh backend catalog/inventory manifest hashes, keeps source rows separate from distinct targets, and shows the audit epoch, dependency window and unresolved reasons. A missing, mismatched or invalid report produces no totals; retrying clears the previous report. Pages never requests this full-backend audit. The summary is bounded at 64 KiB and does not download the complete source inventory or certify whole-window numerical accuracy. Native coverage-report screens are not yet implemented.
